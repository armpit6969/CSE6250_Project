            ...
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                ...
                for index, row in diagnoses_df.iterrows():
                    ...
                    if code in code_to_group:
                        ...
                        found = False
                        for _ in range(1, 5):  # Try up to 4 additional leading zeros
                            try:
                                new_code = code.zfill(len(code) + _)
                                # print(f"Code: {new_code}")
                                group = code_to_group[new_code]
                                found = True
                                break
                            except KeyError:
                                continue
                        if not found:
                            raise KeyError(f"Code {code} not found in code_to_group mapping after retries.")
